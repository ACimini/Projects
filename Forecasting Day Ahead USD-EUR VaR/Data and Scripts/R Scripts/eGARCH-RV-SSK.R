...
eGARCH_RV_SSK <- data[, 1:3]
eGARCH_RV_SSK$ES0.01 <- NA
eGARCH_RV_SSK$ES0.025 <- NA
eGARCH_RV_SSK$ES0.05 <- NA
eGARCH_RV_SSK$ES0.95 <- NA
eGARCH_RV_SSK$ES0.975 <- NA
eGARCH_RV_SSK$ES0.99 <- NA
...
for (i in start_day:(total_days - 1)) {
  ...
  egarch_spec <- ugarchspec(
    ...
    distribution.model = "sstd"  # Using Johnson's SU distribution
  )
  ...
  VaR01 <- qdist("sstd", p = 0.01, mu = fitted(forecast), sigma = sigma(forecast), 
                 skew = coef(fit)["skew"], shape = coef(fit)["shape"])
  ES_01 <- mean(window_data$RR[window_data$RR < VaR01], na.rm = TRUE)
  
  VaR025 <- qdist("sstd", p = 0.025, mu = fitted(forecast), sigma = sigma(forecast), 
                  skew = coef(fit)["skew"], shape = coef(fit)["shape"])
  ES_025 <- mean(window_data$RR[window_data$RR < VaR025], na.rm = TRUE)
  
  VaR05 <- qdist("sstd", p = 0.05, mu = fitted(forecast), sigma = sigma(forecast), 
                 skew = coef(fit)["skew"], shape = coef(fit)["shape"])
  ES_05 <- mean(window_data$RR[window_data$RR < VaR05], na.rm = TRUE)
  
  # Calculate VaR for the upper tail (0.95, 0.975, 0.99)
  VaR95 <- qdist("sstd", p = 0.95, mu = fitted(forecast), sigma = sigma(forecast), 
                 skew = coef(fit)["skew"], shape = coef(fit)["shape"])
  ES_95 <- mean(window_data$RR[window_data$RR > VaR95], na.rm = TRUE)  # Adjusted condition
  
  VaR975 <- qdist("sstd", p = 0.975, mu = fitted(forecast), sigma = sigma(forecast), 
                  skew = coef(fit)["skew"], shape = coef(fit)["shape"])
  ES_975 <- mean(window_data$RR[window_data$RR > VaR975], na.rm = TRUE)  # Adjusted condition
  
  VaR99 <- qdist("sstd", p = 0.99, mu = fitted(forecast), sigma = sigma(forecast), 
                 skew = coef(fit)["skew"], shape = coef(fit)["shape"])
  ES_99 <- mean(window_data$RR[window_data$RR > VaR99], na.rm = TRUE)  # Adjusted condition
  
  # Store the ES predictions
  eGARCH_RV_SSK$ES0.01[i + 1] <- ES_01
  eGARCH_RV_SSK$ES0.025[i + 1] <- ES_025
  eGARCH_RV_SSK$ES0.05[i + 1] <- ES_05
  
  eGARCH_RV_SSK$ES0.95[i + 1] <- ES_95
  eGARCH_RV_SSK$ES0.975[i + 1] <- ES_975
  eGARCH_RV_SSK$ES0.99[i + 1] <- ES_99
...
predicted_ES <- eGARCH_RV_SSK$ES0.01[start_day:total_days]  # Predicted ES values
...
valid_data <- eGARCH_RV_SSK %>% filter(!is.na(ES0.01))
...
write.csv(eGARCH_RV_SSK, file = "eGARCH_RV_SSK.csv")
