...
compute_rolling_var_es_multiple_tau <- function(data, window_size = 1500, taus = c(0.05, 0.025, 0.01)) {
  ...
  for (tau in taus) {
    ...
    for (i in (window_size + 1):nrow(data)) {
      ...
      qr_model <- rq(returns ~  atm1W +  atm1M, 
                     data = window_data, 
                     tau = tau)
      
      next_day_data <- data.frame(atm1W = data$atm1W[i], atm1M = data$atm1M[i])
      # Predict the returns for the current window (distribution of returns)
      predicted_returns <- predict(qr_model, newdata = next_day_data)
      
      # VaR is the return at the given quantile (i.e., the quantile prediction of returns)
      var_values[i] <- predicted_returns
      
      # For ES, we compute the expected shortfall as the mean of returns below VaR
...
write.csv(data1, file = "QR-IV.csv", row.names = FALSE)
