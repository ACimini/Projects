...
data <- data %>%
  select(date, Close, RR, RealizedVariance) %>%  # Select relevant columns
...
compute_rolling_var_es_multiple_tau <- function(data, window_size = 1500, taus = c(0.05, 0.025, 0.01)) {
  ...
  for (tau in taus) {
    ...
    for (i in (window_size + 1):nrow(data)) {
      ...
      term1 <- (pi_val*3*N) / (9*pi_val + 72 -  52*sqrt(3))
      term2 <- (N / (N - 2))
      
      sum_medians <- 0
      for (j in 6:(N - 1)) {
        # Extract the absolute values of the returns at j-1, j, j+1
        med_vals <- abs(c(window_data$RealizedVariance[j-1], 
                          window_data$RealizedVariance[j], window_data$RealizedVariance[j+1]))
        # Compute the median of absolute returns and square it
        med_squared <- median(med_vals)^4
        sum_medians <- sum_medians + med_squared
        
        MedRQ[j] <- term1 * term2 * sum_medians
      }
      # Add CC_t and JC_t to the window data
      CQ_t = CC_t * sqrt(MedRQ)
      
      window_data$CC_t <- CC_t
      window_data$CQ_t <- CQ_t
      
      # Fit the quantile regression model on the rolling window data
      qr_model <- rq(returns ~ RealizedVariance + CC_t + CQ_t, data = window_data, tau = tau)
      ...
      next_day_data <- data.frame(RealizedVariance = data$RealizedVariance[i], 
                                  CC_t = CC_t[N], CQ_t = CQ_t[N])  # Use the last value from the window
...
write.csv(data1, file = "QR-RV-CQ.csv", row.names = FALSE)
