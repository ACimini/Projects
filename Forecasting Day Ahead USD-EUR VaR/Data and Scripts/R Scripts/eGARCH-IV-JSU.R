...
# Function to run the regression and extract coefficients
boot_fn <- function(data, indices) {
  sample_data <- data[indices, ]
  model <- lm(losses ~ predicted_ES, data = sample_data)
  return(coef(model))
}

# Perform bootstrapping
boot_results <- boot(data = data.frame(losses, predicted_ES), statistic = boot_fn, R = 1000)
...
write.csv(eGARCH_IV_JSU, file = "eGARCH_IV_JSU.csv", row.names = FALSE)
