...
compute_rolling_var_es_multiple_tau <- function(data, window_size = 1500, taus = c(0.05, 0.025, 0.01)) {
  ...
  for (tau in taus) {
    ...
    for (i in (window_size + 1):nrow(data)) {
      ...
      qr_model <- rq(returns ~ RealizedVariance, 
                     data = window_data, 
                     tau = tau)
      
      # Prepare the predictor values for the next day (i.e., day i) based on the previous day's values (i-1)
      next_day_data <- data.frame(RealizedVariance = data$RealizedVariance[i])  # Ensure it's a data frame with column name
...
write.csv(data1, file = "QR-RV.csv", row.names = FALSE)
# ---
