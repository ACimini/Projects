...
compute_rolling_var_es_multiple_tau <- function(data, window_size = 1500, taus = c(0.05, 0.025, 0.01)) {
  ...
  for (tau in taus) {
    ...
    for (i in (window_size + 1):nrow(data)) {
      ...
      window_data$SJ_t = window_data$PV_t - window_data$NV_t
      # Fit the quantile regression model on the rolling window data
      qr_model <- rq(returns ~ RealizedVariance + SJ_t + CC_t, 
                     data = window_data, 
                     tau = tau)
      
      # Prepare the predictor values for the next day (i.e., day i) based on the previous day's values (i-1)
      next_day_data <- data.frame(RealizedVariance = data$RealizedVariance[i], SJ_t = 
                                    window_data$SJ_t[N], 
                                  CC_t = window_data$CC_t[N])  # Ensure it's a data frame with column name
...
write.csv(data1, file = "QR-RV-SJ.csv", row.names = FALSE)
# ---
