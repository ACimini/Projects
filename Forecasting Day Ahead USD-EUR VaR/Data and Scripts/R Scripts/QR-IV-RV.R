...
data <- data %>%
  select(date, Close, RR, RealizedVariance, atm1W, atm1M) %>%  # Select relevant columns
...
compute_rolling_var_es_multiple_tau <- function(data, window_size = 1500, taus = c(0.05, 0.025, 0.01)) {
  ...
  for (tau in taus) {
    ...
    for (i in (window_size + 1):nrow(data)) {
      ...
      qr_model <- rq(returns ~ RealizedVariance + atm1W + atm1M, 
                     data = window_data, 
                     tau = tau)
      
      # Prepare the predictor values for the next day (i.e., day i) based on the previous day's values (i-1)
      next_day_data <- data[i, c("atm1W", "atm1M", "RealizedVariance")]
...
write.csv(data1, file = "QR-IV-RV.csv", row.names = FALSE)
# ---
